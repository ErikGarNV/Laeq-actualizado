...
/*
  ...
  <Contact />   // renders Contact + Footer as one block
*/
...
const inputStyle = (focused) => ({
  ...
  border: `1px solid ${focused ? T.cyan + '66' : T.brand + '33'}`,
...
const ContactForm = () => {
  ...
  const send = () => {
    ...
    setLoading(true);
    /* In production: POST to your API / EmailJS / Resend */
  ...
  if (sent) return (
    ...
    >
      ...
      >
        <svg width="36" height="36" viewBox="0 0 24 24" fill="none" stroke={T.cyan} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
          <polyline points="20,6 9,17 4,12"/>
        </svg>
      </motion.div>
      <motion.div initial={{ opacity:0, y:16 }} animate={{ opacity:1, y:0 }} transition={{ delay:0.3 }}>
        <h4 style={{ fontFamily:'"Cormorant Garamond",serif', fontWeight:700, fontSize:28, color:T.white, margin:'0 0 12px', letterSpacing:'-0.02em' }}>
          Consulta recibida
        </h4>
        <p style={{ fontFamily:'"DM Sans",sans-serif', fontWeight:300, fontSize:14.5, color:T.muted, lineHeight:1.75, maxWidth:340, margin:'0 auto 24px' }}>
          Luis Espinoza y su equipo responderán a <strong style={{ color:T.white, fontWeight:500 }}>{form.email}</strong> dentro de las próximas <strong style={{ color:T.cyan, fontWeight:500 }}>24 horas hábiles</strong>.
        </p>
  ...
  return (
    <div style={{ padding:'clamp(28px,4vw,48px)' }}>
      ...
      <div style={{ marginBottom:28 }}>
        ...
        <p style={{ fontFamily:'"DM Sans",sans-serif', fontWeight:300, fontSize:12.5, color:T.dim, margin:0, lineHeight:1.7 }}>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:14 }}>
        <Field label="Nombre completo" required>
          <input type="text" placeholder="Luis García" value={form.nombre} onChange={e => set('nombre',e.target.value)} {...fo('nombre')} style={inputStyle(focused==='nombre')}/>
        </Field>
        <Field label="Email corporativo" required>
          <input type="email" placeholder="su@empresa.com" value={form.email} onChange={e => set('email',e.target.value)} {...fo('email')} style={inputStyle(focused==='email')}/>
        </Field>
        <Field label="Empresa / Organización">
          <input type="text" placeholder="Nombre de su empresa" value={form.empresa} onChange={e => set('empresa',e.target.value)} {...fo('empresa')} style={inputStyle(focused==='empresa')}/>
        </Field>
        <Field label="Teléfono">
          <input type="tel" placeholder="+51 999 779 580" value={form.tel} onChange={e => set('tel',e.target.value)} {...fo('tel')} style={inputStyle(focused==='tel')}/>
        </Field>
        <Field label="Servicio de interés" span2>
          <div style={{ position:'relative' }}>
            <select value={form.servicio} onChange={e => set('servicio',e.target.value)}
              style={{ ...inputStyle(focused==='servicio'), width:'100%', appearance:'none', paddingRight:40, cursor:'pointer', background:form.servicio ? `${T.brand}18` : `${T.brand}0A`, color: form.servicio ? T.white : T.dim + 'BB' }}
              {...fo('servicio')}
            >
              <option value="" style={{ background:'#071E30' }}>Selecciona un servicio…</option>
              {SERVICES.map(s => <option key={s} value={s} style={{ background:'#071E30', color:T.white }}>{s}</option>)}
            </select>
        ...
        <Field label="Mensaje / Consulta" required span2>
          <textarea rows={4} placeholder="Describa su consulta, proyecto o necesidad específica…" value={form.mensaje} onChange={e => set('mensaje',e.target.value)} {...fo('mensaje')}
            style={{ ...inputStyle(focused==='mensaje'), resize:'vertical', lineHeight:1.7, minHeight:120 }}
          />
        </Field>
      </div>
      ...
      <div style={{ marginTop:20, display:'flex', flexDirection:'column', gap:12 }}>
        <div style={{ display:'flex', alignItems:'flex-start', gap:10 }}>
          ...
          <p style={{ fontFamily:'"DM Mono",monospace', fontSize:8.5, color:T.dim, margin:0, lineHeight:1.55, letterSpacing:'0.08em' }}>
            Sus datos son tratados bajo nuestra Política de Privacidad y la Ley Peruana de Protección de Datos Personales (Ley 29733). Nunca compartimos información con terceros.
          </p>
        </div>
        <motion.button
          onClick={send}
...
const INFO_ITEMS = [
  {
    ...
    lines: ['Calle Raymundo Cárcamo 904, Of. 501', 'Urb. Santa Catalina, La Victoria', 'Lima – Perú'],
    link: 'https://maps.google.com/?q=Calle+Raymundo+Carcamo+904,+La+Victoria,+Lima',
  },
  {
    icon: <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07A19.5 19.5 0 0 1 4.69 12a19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 3.6 1.37h3a2 2 0 0 1 2 1.72 12.84 12.84 0 0 0 .7 2.81 2 2 0 0 1-.45 2.11L8.09 9a16 16 0 0 0 6.29 6.29l.86-.86a2 2 0 0 1 2.11-.45 12.84 12.84 0 0 0 2.81.7A2 2 0 0 1 22 16.92z"/></svg>,
    label: 'Teléfonos',
  ...
  return (
    <section ref={ref} id="contacto" style={{ background:T.bg, paddingTop:'clamp(80px,10vh,120px)', position:'relative', overflow:'hidden' }}>
      ...
      <div style={{ position:'relative', zIndex:1, padding:'0 clamp(24px,5vw,80px) clamp(80px,10vh,120px)' }}>
        ...
        <div style={{ marginBottom:'clamp(44px,6vh,72px)' }}>
          ...
          >
            ...
            <span style={{ fontFamily:'"DM Mono",monospace', fontSize:10, color:T.cyan, textTransform:'uppercase', letterSpacing:'0.42em' }}>Contacto · Lima, Perú</span>
          </motion.div>
          <div style={{ display:'flex', flexWrap:'wrap', alignItems:'flex-end', justifyContent:'space-between', gap:20 }}>
            ...
            >
              Hablemos de{' '}
              <em style={{ color:T.cyan, fontStyle:'italic', fontWeight:600 }}>Energía</em>
            </motion.h2>
            <motion.p
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1.6fr', gap:24, alignItems:'start' }} className="contact-grid">
          ...
          >
            ...
            <div style={{ background:`linear-gradient(145deg, ${T.bgMid} 0%, ${T.bg} 100%)`, border:`1px solid ${T.brand}25`, padding:'clamp(22px,3vw,36px)', position:'relative', overflow:'hidden' }}>
              ...
              <h4 style={{ fontFamily:'"DM Sans",sans-serif', fontWeight:600, fontSize:12, color:T.white, margin:'0 0 22px', textTransform:'uppercase', letterSpacing:'0.22em', position:'relative', zIndex:1 }}>
                  ...
                  <div key={item.label}>
                    {item.link ? (
                      <a href={item.link} target={item.link.startsWith('http') ? '_blank' : '_self'} rel="noopener noreferrer"
                        ...
                        onMouseEnter={e => e.currentTarget.querySelector('.info-label').style.color=T.cyan}
                        onMouseLeave={e => e.currentTarget.querySelector('.info-label').style.color=T.white}
                      >
                        <div style={{ width:34, height:34, flexShrink:0, display:'flex', alignItems:'center', justifyContent:'center', background:`${T.brand}22`, border:`1px solid ${T.brand}33`, color:T.cyan }}>
                          {item.icon}
                        </div>
                        <div>
                          <p className="info-label" style={{ fontFamily:'"DM Sans",sans-serif', fontWeight:600, fontSize:11.5, color:T.white, margin:'0 0 3px', transition:'color .25s' }}>{item.label}</p>
                          {item.lines.map(l => <p key={l} style={{ fontFamily:'"DM Sans",sans-serif', fontWeight:300, fontSize:12, color:T.muted, margin:'0', lineHeight:1.55 }}>{l}</p>)}
                        </div>
                      </a>
                    ) : (
                      <div style={{ display:'flex', alignItems:'flex-start', gap:14 }}>
                        ...
                        <div>
                          <p style={{ fontFamily:'"DM Sans",sans-serif', fontWeight:600, fontSize:11.5, color:T.white, margin:'0 0 3px' }}>{item.label}</p>
                          {item.lines.map(l => <p key={l} style={{ fontFamily:'"DM Sans",sans-serif', fontWeight:300, fontSize:12, color:T.muted, margin:0, lineHeight:1.55 }}>{l}</p>)}
                        </div>
                      </div>
                    )}
            ...
            <motion.div
              onClick={() => navigate('/libro-de-reclamaciones')}
              whileHover={{ scale:1.01 }}
              whileTap={{ scale:0.98 }}
              data-c="reclamaciones"
              style={{
            ...
            >
              ...
              {/* Book icon — filtered for dark bg */}
              <div style={{ flexShrink:0, position:'relative', zIndex:1 }}>
                <img src={libroImg} alt="Libro de Reclamaciones"
                  style={{ width:56, height:56, objectFit:'contain', filter:'invert(1) brightness(0.7) sepia(1) hue-rotate(170deg) saturate(2)', opacity:0.75 }}
                />
              </div>
              ...
              <div style={{ position:'relative', zIndex:1, flex:1 }}>
                ...
                <p style={{ fontFamily:'"DM Mono",monospace', fontSize:8.5, color:`${T.active}66`, margin:0, textTransform:'uppercase', letterSpacing:'0.16em', lineHeight:1.4 }}>Código del Consumidor · Ley 29571</p>
              </div>

              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke={T.active} strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round" style={{ flexShrink:0, position:'relative', zIndex:1 }}>
      ...
      <style>{`
        ...
        input::placeholder, textarea::placeholder, select::placeholder { color: rgba(184,223,240,0.28) !important; }
      `}</style>
    </section>
  );
};
...
const FOOTER_LINKS = {
  ...
  'Empresa':   ['Nosotros','Proyectos','Publicaciones','Clientes','Contacto'],
...
const Footer = () => {
  ...
  return (
    <footer style={{ background:T.bgDeep, borderTop:`1px solid ${T.brand}18`, position:'relative', overflow:'hidden' }}>
      ...
      <div style={{ position:'relative', zIndex:1, maxWidth:'120rem', margin:'0 auto' }}>
        ...
        {/* UPPER */}
        <div style={{ padding:'clamp(52px,7vh,88px) clamp(24px,5vw,80px) clamp(36px,5vh,56px)', display:'grid', gridTemplateColumns:'1.8fr 1fr 1fr 1fr', gap:'clamp(28px,4vw,56px)' }} className="footer-grid">

          {/* Brand col */}
          <div>
            <div style={{ display:'flex', alignItems:'center', gap:12, marginBottom:20 }}>
              <img src={logoLaeq} alt="LAEQ"
                style={{ height:44, width:'auto', objectFit:'contain', filter:`drop-shadow(0 0 10px ${T.cyan}44)` }}
              />
              <div>
            ...
            <p style={{ fontFamily:'"DM Sans",sans-serif', fontWeight:300, fontSize:13, color:T.dim, lineHeight:1.75, maxWidth:280, margin:'0 0 24px' }}>
            ...
            <div style={{ display:'flex', gap:10 }}>
              {[
                { label:'LinkedIn', url:'https://www.linkedin.com/company/luis-espinoza-&-asociados', icon:<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M16 8a6 6 0 0 1 6 6v7h-4v-7a2 2 0 0 0-2-2 2 2 0 0 0-2 2v7h-4v-7a6 6 0 0 1 6-6z"/><rect x="2" y="9" width="4" height="12"/><circle cx="4" cy="4" r="2"/></svg> },
                { label:'Email', url:'mailto:gerencia@laeqasociados.com', icon:<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/><polyline points="22,6 12,13 2,6"/></svg> },
                { label:'Teléfono', url:'tel:+5112775502', icon:<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07A19.5 19.5 0 0 1 4.69 12a19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 3.6 1.37h3a2 2 0 0 1 2 1.72c.127.96.361 1.903.7 2.81a2 2 0 0 1-.45 2.11L8.09 9a16 16 0 0 0 6.29 6.29l.86-.86a2 2 0 0 1 2.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0 1 22 16.92z"/></svg> },
              ].map(s => (
                <a key={s.label} href={s.url} target={s.url.startsWith('http') ? '_blank' : '_self'} rel="noopener noreferrer"
                  style={{ width:36, height:36, display:'flex', alignItems:'center', justifyContent:'center', background:`${T.brand}15`, border:`1px solid ${T.brand}30`, color:T.muted, textDecoration:'none', transition:'all .25s' }}
                  ...
                  onMouseLeave={e => { e.currentTarget.style.borderColor=`${T.brand}30`; e.currentTarget.style.color=T.muted; e.currentTarget.style.background=`${T.brand}15`; }}
                >{s.icon}</a>
              ))}
            </div>
          </div>
          ...
          {Object.entries(FOOTER_LINKS).map(([title, links]) => (
            <div key={title}>
              ...
              <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
                {links.map(l => (
                  <span key={l}
                    onClick={() => l === 'Libro de Reclamaciones' ? navigate('/libro-de-reclamaciones') : undefined}
                    style={{ fontFamily:'"DM Sans",sans-serif', fontWeight:300, fontSize:13, color:T.dim, cursor:'pointer', transition:'color .25s', width:'fit-content', lineHeight:1.3 }}
        ...
        {/* LOWER */}
        <div style={{ padding:'clamp(18px,2.5vh,24px) clamp(24px,5vw,80px)', borderTop:`1px solid ${T.brand}14`, display:'flex', flexWrap:'wrap', alignItems:'center', justifyContent:'space-between', gap:14 }}>
          <div style={{ display:'flex', flexWrap:'wrap', gap:6, alignItems:'center' }}>
            <span style={{ fontFamily:'"DM Mono",monospace', fontSize:9, color:`${T.active}44`, letterSpacing:'0.14em' }}>© {year}</span>
            <span style={{ fontFamily:'"DM Mono",monospace', fontSize:9, color:`${T.active}66`, letterSpacing:'0.14em' }}>Luis Espinoza Quiñones & Asociados S.A.C.</span>
            <span style={{ fontFamily:'"DM Mono",monospace', fontSize:9, color:`${T.brand}55`, letterSpacing:'0.1em' }}>· RUC 20563654431 ·</span>
            <span style={{ fontFamily:'"DM Mono",monospace', fontSize:9, color:`${T.brand}44`, letterSpacing:'0.1em' }}>Lima, Perú</span>
          </div>
          <div style={{ display:'flex', gap:20, alignItems:'center' }}>
            <span style={{ fontFamily:'"DM Mono",monospace', fontSize:8.5, color:`${T.brand}44`, textTransform:'uppercase', letterSpacing:'0.16em', cursor:'pointer', transition:'color .25s' }}
              onMouseEnter={e => e.currentTarget.style.color=T.muted}
              onMouseLeave={e => e.currentTarget.style.color=`${T.brand}44`}
            >Privacidad</span>
            <span style={{ fontFamily:'"DM Mono",monospace', fontSize:8.5, color:`${T.brand}44`, textTransform:'uppercase', letterSpacing:'0.16em', cursor:'pointer', transition:'color .25s' }}
              onMouseEnter={e => e.currentTarget.style.color=T.muted}
              onMouseLeave={e => e.currentTarget.style.color=`${T.brand}44`}
            ...
            <span
              onClick={() => navigate('/libro-de-reclamaciones')}
              style={{ fontFamily:'"DM Mono",monospace', fontSize:8.5, color:`${T.active}55`, textTransform:'uppercase', letterSpacing:'0.16em', cursor:'pointer', transition:'color .25s', display:'flex', alignItems:'center', gap:5 }}
              onMouseEnter={e => e.currentTarget.style.color=T.cyan}
              onMouseLeave={e => e.currentTarget.style.color=`${T.active}55`}
            >
              <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M4 19.5A2.5 2.5 0 0 1 6.5 17H20"/><path d="M6.5 2H20v20H6.5A2.5 2.5 0 0 1 4 19.5v-15A2.5 2.5 0 0 1 6.5 2z"/></svg>
