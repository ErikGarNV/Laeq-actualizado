...
/* ─── CONTACT FORM (Awwwards interaction) ─────── */
/*
  When user clicks "Solicitar Información":
  The modal smoothly morphs into a contact form
  pre-filled with the project context.
  The form IS the CTA — no dead buttons.
*/
      ...
      >
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}>
          ...
          <button onClick={onClose} style={{
            ...
            color: T.bg, textTransform: 'uppercase', letterSpacing: '0.2em',
          }}>Cerrar</button>
        </motion.div>
      </motion.div>
    );
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          {[
            ...
            <div key={key} style={{ position: 'relative' }}>
              <input
                ...
                onBlur={e => e.target.style.borderColor = `${T.brand}33`}
              />
            </div>
          ))}
        </div>
        ...
        <textarea
          ...
          style={{
            ...
            background: `${T.brand}0A`,
            border: `1px solid ${T.brand}33`,
            fontFamily: '"DM Sans",sans-serif', fontSize: 12.5, color: T.muted,
        ...
        {/* Context tag */}
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 12px', background: `${p.color}10`, border: `1px solid ${p.color}25` }}>
          <div style={{ width: 5, height: 5, borderRadius: '50%', background: p.color, flexShrink: 0 }} />
          <span style={{
            fontFamily: '"DM Mono",monospace', fontSize: 9, color: `${p.color}BB`,
    ...
    >
      <motion.div
        ...
        style={{
          ...
          position: 'relative',
          scrollbarWidth: 'thin',
          scrollbarColor: `${T.brand}44 transparent`,
        }}
      >
        {/* Accent line */}
        <motion.div
          ...
          style={{
            ...
            background: `linear-gradient(90deg, ${p.color} 0%, ${T.brand}55 65%, transparent 100%)`,
        ...
        <AnimatePresence mode="wait">
          {showForm ? (
            ...
            >
              ...
              <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 24, gap: 16 }}>
                <div>
                  <div style={{ display: 'flex', alignItems: 'center', flexWrap: 'wrap', gap: 8, marginBottom: 10 }}>
                    <span style={{ fontFamily: '"DM Mono",monospace', fontSize: 9, background: `${p.color}20`, color: p.color, border: `1px solid ${p.color}44`, padding: '4px 10px', textTransform: 'uppercase', letterSpacing: '0.18em' }}>{p.cat}</span>
                    <span style={{ fontFamily: '"DM Mono",monospace', fontSize: 9, color: `${T.active}66`, textTransform: 'uppercase', letterSpacing: '0.16em' }}>{p.client}</span>
                    <span style={{ fontFamily: '"DM Mono",monospace', fontSize: 9, color: `${T.active}44`, letterSpacing: '0.14em' }}>{p.year}</span>
                  </div>
                ...
                <button onClick={onClose} style={{ background: 'none', border: `1px solid rgba(30,184,240,0.18)`, width: 44, height: 44, flexShrink: 0, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'border-color .3s' }}
                  onMouseEnter={e => e.currentTarget.style.borderColor = T.cyan}
                  onMouseLeave={e => e.currentTarget.style.borderColor = 'rgba(30,184,240,0.18)'}
                >
              ...
              {/* KPI banner */}
              <div style={{ display: 'flex', alignItems: 'center', gap: 20, padding: '14px 18px', background: `${p.color}10`, border: `1px solid ${p.color}28`, marginBottom: 24 }}>
                ...
                <div style={{ width: 1, height: 36, background: `${p.color}30` }} />
                <div>
                  <p style={{ fontFamily: '"DM Mono",monospace', fontSize: 8.5, color: `${T.active}55`, textTransform: 'uppercase', letterSpacing: '0.2em', margin: '0 0 2px' }}>Resultado clave</p>
              ...
              <div style={{ height: 1, background: `${T.brand}25`, marginBottom: 24 }} />
              ...
              <div style={{ marginBottom: 24 }}>
                ...
                <p style={{ fontFamily: '"DM Sans",sans-serif', fontWeight: 300, fontSize: 'clamp(12.5px,1vw,14px)', color: T.dim, lineHeight: 1.85, margin: 0 }}>{p.scope}</p>
              </div>

              {/* Results */}
              <div style={{ background: `${T.brand}08`, border: `1px solid ${T.brand}20`, padding: 'clamp(14px,2.5vw,24px)', marginBottom: 24 }}>
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginBottom: 24 }}>
                  {p.links.map(lk => (
                    <a key={lk.url} href={lk.url} target="_blank" rel="noopener noreferrer"
                      ...
                      onMouseLeave={e => { e.currentTarget.style.borderColor = `${T.brand}33`; e.currentTarget.style.color = T.muted; }}
                    >
              ...
              <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
                {/* ← THE MAGIC BUTTON: morphs to contact form */}
                <motion.button
                  ...
                  onMouseLeave={e => e.currentTarget.style.background = p.color}
                >
                ...
                <button onClick={onClose} style={{ padding: '13px 22px', background: 'transparent', border: `1px solid ${T.brand}`, cursor: 'pointer', fontFamily: '"DM Sans",sans-serif', fontWeight: 400, fontSize: 10.5, color: T.muted, textTransform: 'uppercase', letterSpacing: '0.2em', transition: 'all .28s' }}
                  onMouseEnter={e => { e.currentTarget.style.borderColor = T.cyan; e.currentTarget.style.color = T.cyan; }}
...
const Card = ({ p, onOpen, inView, i }) => {
  ...
  return (
    <motion.article
      ...
      style={{
        background: hov
          ? `linear-gradient(145deg, ${T.bgCard} 0%, ${T.bg} 100%)`
          : `linear-gradient(145deg, ${T.bgCard} 0%, ${T.bgDeep} 100%)`,
        border: `1px solid ${hov ? p.color + '55' : T.brand + '22'}`,
    ...
    >
      ...
      />
      {/* Corner glow */}
      {hov && <div style={{ position: 'absolute', top: '-30%', right: '-10%', width: '55%', height: '75%', pointerEvents: 'none', background: `radial-gradient(ellipse, ${p.color}14 0%, transparent 65%)` }} />}

      {/* Meta row */}
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 16 }}>
        <div>
          <span style={{ fontFamily: '"DM Mono",monospace', fontSize: 8.5, color: `${p.color}CC`, textTransform: 'uppercase', letterSpacing: '0.22em', display: 'block', marginBottom: 3 }}>{p.cat}</span>
          <span style={{ fontFamily: '"DM Mono",monospace', fontSize: 8.5, color: `${T.active}55`, letterSpacing: '0.12em' }}>{p.client}</span>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
          <span style={{ fontFamily: '"DM Mono",monospace', fontSize: 8.5, color: `${T.brand}88`, letterSpacing: '0.1em' }}>{p.year}</span>
          <motion.div animate={{ x: hov ? 4 : 0, color: hov ? p.color : T.brand + '55' }} transition={{ duration: 0.3, ease: SILK }} style={{ lineHeight: 0 }}>
      ...
      {/* Summary */}
      <p style={{ fontFamily: '"DM Sans",sans-serif', fontWeight: 300, fontSize: 12, color: T.dim, lineHeight: 1.7, margin: '0 0 auto', flex: 1 }}>
        {p.summary.slice(0, 100)}…
      </p>

      {/* Bottom */}
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginTop: 18, paddingTop: 14, borderTop: `1px solid ${hov ? p.color + '28' : T.brand + '15'}`, transition: 'border-color .35s' }}>
        <div>
          ...
          <div style={{ fontFamily: '"DM Mono",monospace', fontSize: 8, color: `${T.active}55`, textTransform: 'uppercase', letterSpacing: '0.14em', marginTop: 3 }}>{p.kpi.label}</div>
        </div>
        <AnimatePresence>
          {hov && (
            <motion.div
              ...
              style={{ padding: '7px 14px', background: p.color, fontFamily: '"DM Sans",sans-serif', fontWeight: 600, fontSize: 9.5, color: T.bg, textTransform: 'uppercase', letterSpacing: '0.16em', display: 'flex', alignItems: 'center', gap: 6 }}
            >
              Ver detalles
...
const Pill = ({ label, active, onClick }) => (
  <button onClick={onClick} data-c="" style={{
    padding: '8px 18px', background: active ? T.cyan : 'transparent',
    border: `1px solid ${active ? T.cyan : T.brand + '44'}`,
    ...
    onMouseEnter={e => { if (!active) { e.currentTarget.style.borderColor = T.cyan + '66'; e.currentTarget.style.color = T.white; } }}
    onMouseLeave={e => { if (!active) { e.currentTarget.style.borderColor = T.brand + '44'; e.currentTarget.style.color = T.muted; } }}
  >{label}</button>
);
...
const StatsBar = ({ inView }) => (
  <motion.div
    ...
    style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', border: `1px solid ${T.brand}28`, marginTop: 'clamp(44px,5vh,64px)' }}
  ...
  return (
    <>
      <section ref={ref} id="proyectos" style={{ background: T.bg, paddingTop: 'clamp(80px,10vh,120px)', paddingBottom: 'clamp(80px,10vh,120px)', position: 'relative', overflow: 'hidden' }}>
        {/* Grid texture */}
        <div style={{ position: 'absolute', inset: 0, pointerEvents: 'none', zIndex: 0, opacity: 0.4, backgroundImage: `linear-gradient(${T.brand}08 1px, transparent 1px), linear-gradient(90deg, ${T.brand}08 1px, transparent 1px)`, backgroundSize: '72px 72px' }} />
        <div style={{ position: 'absolute', right: '-5%', top: '15%', width: '40vw', height: '50vh', pointerEvents: 'none', zIndex: 0, background: `radial-gradient(ellipse, ${T.brand}14 0%, transparent 60%)` }} />

        <div style={{ position: 'relative', zIndex: 1, padding: '0 clamp(24px,5vw,80px)' }}>

          {/* Header */}
          <div style={{ marginBottom: 'clamp(36px,5vh,56px)' }}>
            <motion.div initial={{ opacity: 0, x: -16 }} animate={inView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.7, ease: SILK }} style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 16 }}>
              <div style={{ width: 28, height: 1, background: T.cyan }} />
              <span style={{ fontFamily: '"DM Mono",monospace', fontSize: 10, color: T.cyan, textTransform: 'uppercase', letterSpacing: '0.42em' }}>Casos de Éxito · Desde 2014</span>
            </motion.div>
            <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'flex-end', justifyContent: 'space-between', gap: 20 }}>
              <motion.h2 initial={{ opacity: 0, y: 20 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.9, ease: SILK, delay: 0.1 }} style={{ fontFamily: '"Cormorant Garamond",serif', fontWeight: 700, fontSize: 'clamp(42px,6.5vw,88px)', color: T.white, margin: 0, lineHeight: 1, letterSpacing: '-0.025em', paddingBottom: '0.08em' }}>
                Proyectos{' '}<em style={{ color: T.cyan, fontStyle: 'italic', fontWeight: 600 }}>Destacados</em>
              </motion.h2>
              <motion.p initial={{ opacity: 0 }} animate={inView ? { opacity: 1 } : {}} transition={{ duration: 0.8, ease: SILK, delay: 0.22 }} style={{ fontFamily: '"DM Sans",sans-serif', fontWeight: 300, fontSize: 13, color: T.muted, lineHeight: 1.75, maxWidth: 300, margin: 0 }}>
          ...
          {/* Filters */}
          <motion.div initial={{ opacity: 0, y: 10 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.7, ease: SILK, delay: 0.18 }} style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginBottom: 'clamp(28px,4vh,48px)' }}>
          ...
          {/* Bottom CTA */}
          <motion.div initial={{ opacity: 0, y: 18 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.8, ease: SILK, delay: 0.4 }} style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'space-between', gap: 20, padding: 'clamp(32px,5vh,52px) 0 0', borderTop: `1px solid ${T.brand}20`, marginTop: 'clamp(36px,5vh,56px)' }}>
            <div>
      ...
      <style>{`
        ...
        input::placeholder, textarea::placeholder { color: rgba(184,223,240,0.28); }
